...
d3.csv('static/data/state_stats.csv').then(function(data) { //states_sightings.csv is csv data that contains state ufo sightings
  ...
  var ufoData = "static/data/ufo_data.csv";

  d3.csv(ufoData, function(d) {

    for (var i = 0; i < d.length; i++) {

      var latitude = parseFloat(d[i].latitude);
      var longitude = parseFloat(d[i].longitude);
      console.log(latitude, longitude);
      console.log('hello');
      location = [latitude, londitude];
